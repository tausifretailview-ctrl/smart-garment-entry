...
- **Display dedupe stays.** `reconcileSaleInvoiceDisplay` (DUPLICATE_CN_PAID_MATCH_TOL) is retained for legacy rows where both `sale_return_adjust` and a CN voucher exist; do not remove until `FloatingSaleReturn` POS-redeem path is migrated to the unified RPC (Phase 2).
- **Phase 2 (not shipped):** deprecate `apply_credit_note_to_sale` in favor of a single entry point; migrate `useCreditNotes` + `FloatingSaleReturn` pending-CN-redeem flow.
