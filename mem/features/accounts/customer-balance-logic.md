...
**Planned consistency fix (CN RPC + vouchers):** see `docs/customer-accounts-consistency-v1.md` — Option A keeps post-hoc CN on `sale_return_adjust`; RPC becomes single voucher writer; do not move to `paid_amount`-only without reconciliation changes.
