-- Align `get_customer_true_outstanding` with POS pay-at-sale tender on sales rows
-- (cash_amount / card_amount / upi_amount) when no matching receipt voucher exists.
...
BEGIN
  ...
  SELECT
    ...
    'POS pay-at-sale (paid_amount or cash/card/upi) minus receipt vouchers on same sale'::text
  FROM (
    SELECT GREATEST(
      0::numeric,
      GREATEST(
      ...
      - COALESCE((
        SELECT SUM(GREATEST(0::numeric, COALESCE(ve.total_amount, 0)))
        FROM public.voucher_entries ve
        WHERE ve.organization_id = p_organization_id
          AND ve.deleted_at IS NULL
          AND lower(COALESCE(ve.voucher_type, '')) = 'receipt'
...
COMMENT ON FUNCTION public.reconcile_customer_balance(uuid, uuid) IS
  'Lifetime outstanding components: SUM(amount) = true outstanding. Includes paid_at_sale_drift for POS cash/UPI on sales without receipt vouchers.';
