-- Align paid_diverges_from_receipts with compute_sale_settlement (canonical paid_amount).
-- Previous definition compared paid_amount to raw SUM(voucher totals) and skipped
-- legacy_paid_baseline rows — a second formula that drifts from the app writer and
-- missed delivery of understated-paid defects (see cn_over_apply_repair_20260606).
--
-- Also adds get_paid_settlement_mismatch_digest() for per-org counts + top-10 worst rows
-- used by run-invariant-digest WhatsApp/email alerts.
...
UNION ALL
-- paid_amount must equal compute_sale_settlement(...).new_paid (tenders + live receipts,
-- CN-vs-SRA dedupe, net_amount cap). detail = recorded − expected.
...
COMMENT ON VIEW public.v_accounting_invariants IS
  'Accounting invariant rows. paid_diverges_from_receipts compares sales.paid_amount to '
  'compute_sale_settlement.new_paid (not a raw voucher sum).';

-- Per-org failing count, abs rupee discrepancy, and ten worst rows (for alerts / UI).
CREATE OR REPLACE FUNCTION public.get_paid_settlement_mismatch_digest()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  WITH mismatches AS (
    SELECT
      ...
      COALESCE(c.new_paid, 0) AS expected_paid,
      round(COALESCE(s.paid_amount, 0) - COALESCE(c.new_paid, 0), 2) AS discrepancy,
      abs(round(COALESCE(s.paid_amount, 0) - COALESCE(c.new_paid, 0), 2)) AS abs_discrepancy
    FROM public.sales s
    CROSS JOIN LATERAL public.compute_sale_settlement(s.id, s.organization_id) c
    WHERE s.deleted_at IS NULL
      AND COALESCE(s.is_cancelled, false) = false
  ...
  org_agg AS (
    SELECT
      ...
      round(sum(m.abs_discrepancy), 2) AS total_abs_discrepancy
    FROM mismatches m
    ...
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'expected_paid', m.expected_paid,
          'discrepancy', m.discrepancy
        )
        ORDER BY m.abs_discrepancy DESC
      ) AS worst_rows
    FROM (
      SELECT
        m.*,
        row_number() OVER (PARTITION BY m.organization_id ORDER BY m.abs_discrepancy DESC) AS rn
      FROM mismatches m
    ) m
    WHERE m.rn <= 10
    GROUP BY m.organization_id
  ...
  RETURN jsonb_build_object(
    'check_name', 'paid_diverges_from_receipts',
...
COMMENT ON FUNCTION public.get_paid_settlement_mismatch_digest() IS
  'Platform-admin / service-role digest: per-org paid_amount vs compute_sale_settlement mismatches '
  'with failing_count, total_abs_discrepancy, and up to 10 worst rows.';

REVOKE ALL ON FUNCTION public.get_paid_settlement_mismatch_digest() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_paid_settlement_mismatch_digest() TO authenticated, service_role;
