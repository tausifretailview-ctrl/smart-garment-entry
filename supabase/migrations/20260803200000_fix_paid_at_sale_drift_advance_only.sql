-- POS paid_at_sale_drift: only when cash/card/upi tender exists on the sale row.
-- Advance-only paid_amount (no tender) must not drift — it is already in advances_applied.
...
COMMENT ON FUNCTION public.reconcile_customer_balance(uuid, uuid) IS
  'Lifetime outstanding components. paid_at_sale_drift only when cash/card/upi tender > 0 (not advance-only paid_amount).';
