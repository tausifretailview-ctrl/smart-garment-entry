-- Function to detect stock discrepancies
CREATE OR REPLACE FUNCTION public.detect_stock_discrepancies(p_organization_id uuid DEFAULT NULL)
RETURNS TABLE (
  variant_id uuid,
  barcode text,
  product_name text,
...
-- Function to fix stock discrepancies by updating stock_qty to match calculated values
CREATE OR REPLACE FUNCTION public.fix_stock_discrepancies(p_organization_id uuid DEFAULT NULL)
RETURNS TABLE (
  fixed_count integer,
  details jsonb
)
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      v_record.variant_id,
      'reconciliation',
      -v_record.discrepancy,
      'Stock reconciliation: adjusted from ' || v_record.current_stock_qty || ' to ' || v_record.calculated_stock_qty,
      pv.organization_id
    FROM product_variants pv
...
GRANT EXECUTE ON FUNCTION public.fix_stock_discrepancies(uuid) TO authenticated;