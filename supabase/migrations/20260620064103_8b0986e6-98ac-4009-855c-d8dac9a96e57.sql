...
BEGIN
  ...
  WITH filtered_sales AS (
    ...
    WHERE s.organization_id = p_organization_id
      ...
      AND (
        ...
        OR COALESCE(s.credit_applied, 0) > 0
      )
      AND (
        v_refund = 'all'
      ...
      AND (
        v_credit_note = 'all'
        OR (v_credit_note = 'with_credit_note' AND (s.credit_note_id IS NOT NULL OR COALESCE(s.credit_applied, 0) > 0))
        OR (
          v_credit_note = 'without_credit_note'
          AND s.credit_note_id IS NULL
          AND (s.credit_applied IS NULL OR s.credit_applied = 0)
        )
      )
  ),
  sale_metrics AS (
  ...
  SELECT json_build_object(
    ...
    'creditNoteCount', COUNT(*) FILTER (
      WHERE NOT is_hold AND (credit_note_id IS NOT NULL OR COALESCE(credit_applied, 0) > 0)
    )::integer,
    'creditNoteAmount', COALESCE(SUM(
      CASE WHEN NOT is_hold THEN COALESCE(credit_note_amount, credit_applied, 0) ELSE 0 END
    ), 0),
