-- Scope items_gross in get_invoice_dashboard_stats to sales with sale_return_adjust > 0.
--
-- Prior proof (output-identical):
--   docs/party-balances-perf-rewrite-2026-08.md § "items_gross scoping"
--   supabase/migrations/20260823140000_party_balances_rows_v2.sql (items_gross CTE)
--   docs/dashboard-perf-pos-sales-purchase-2026-08.md (optional follow-up for this RPC)
--   supabase/migrations/20260823150000_dashboard_perf_receipts_purchase.sql header
--     (intent for this RPC; body never shipped; Nov 2026 rewrite kept unscoped scan)
--
-- Why it still holds against CURRENT invoice_reconcile_outstanding:
--   items_gross only gates v_sr_applied_on_top when
--     items_gross IS NOT NULL AND items_gross > 0.01 AND sale_return_adjust > 0.01
--     AND (net + sr) > (items_gross + 1).
--   When sale_return_adjust <= 0.01, items_gross (NULL or any value) cannot change
--   outstanding. Dropping those sales from the CTE yields NULL on LEFT JOIN — same path.
--
-- MUST NOT change: receipt attribution, ILIKE matching, or invoice_reconcile_outstanding.
...
BEGIN
  ...
  items_gross AS (
    -- Scoped: only sales where sale_return_adjust > 0 (output-identical; see file header).
    SELECT
      si.sale_id,
  ...
  -- Only vouchers pointing at filtered sales or their customers (uuid IN — indexable).
  -- Do NOT scan all org receipts with description ILIKE sale_number.
  receipt_rows AS (
    SELECT
      ve.reference_id,
      ve.reference_type,
    ...
    WHERE ve.organization_id = p_organization_id
      ...
      AND (
        ve.reference_id IN (SELECT id FROM sale_ids)
        OR ve.reference_id IN (SELECT id FROM customer_ids)
      )
  ),
  receipt_attributed AS (
    ...
    INNER JOIN receipt_rows rr ON (
      -- Direct sale-linked receipt (uuid)
      rr.reference_id = fs.id
      OR (
        -- Customer-scoped receipt mentioning this invoice number
        fs.customer_id IS NOT NULL
        AND rr.reference_id = fs.customer_id
  ...
  reconciled AS (
    SELECT
      rb.*,
      CASE
        WHEN rb.is_cancelled = true OR rb.payment_status = 'cancelled' THEN 'cancelled'::text
        WHEN rb.payment_status = 'hold' THEN 'hold'::text
        WHEN rb.outstanding <= 0.01 THEN 'completed'::text
        WHEN COALESCE(rb.split_cash, 0) > 0.01
          OR COALESCE(rb.split_adv, 0) > 0.01
          OR GREATEST(0, COALESCE(rb.split_cn, 0) - GREATEST(0, COALESCE(rb.sale_return_adjust, 0))) > 0.01
          THEN 'partial'::text
        ELSE 'pending'::text
      END AS derived_status
    FROM reconciled_base rb
  ),
  stats_rows AS (
    SELECT r.*
    FROM reconciled r
...
REVOKE ALL ON FUNCTION public.get_invoice_dashboard_stats(
  uuid,
  timestamptz,
  timestamptz,
  jsonb,
