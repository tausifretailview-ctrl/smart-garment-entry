-- Parity fix: signed_balance must equal get_customer_true_outstanding for ALL customers.
-- 1) sale_receipt_vouchers: join receipt_linkable_sales (deleted_at only), matching
--    reconcile_customer_balance receipt_payments EXISTS — not valid_sales (cancelled/hold).
-- 2) Consolidate advance pool into customer_advance_pools; subtract used + unused_pool inline
--    (same components as reconcile_customer_balance advances_applied + unused_advances).
-- 3) #variable_conflict use_column — avoid PL/pgSQL RETURNS TABLE name shadowing.
...
BEGIN
  ...
  sale_receipt_vouchers AS (
    ...
    INNER JOIN receipt_linkable_sales rls ON rls.id::text = ve.reference_id::text
...
COMMENT ON FUNCTION public.get_customer_party_balances(uuid) IS
  'Set-based customer balance list. signed_balance = SUM(reconcile_customer_balance components) = get_customer_true_outstanding. '
  'Receipt vouchers link to sales via receipt_linkable_sales (matches per-customer reconcile). '
  'advance_available = unused advance pool; net_position = signed_balance − advance_available (secondary).';
