-- Create function to handle sale item updates (stock adjustment on quantity change)
CREATE OR REPLACE FUNCTION handle_sale_item_update()
RETURNS TRIGGER AS $$
DECLARE
...
-- Create the trigger on sale_items table
DROP TRIGGER IF EXISTS trigger_sale_item_update ON sale_items;
CREATE TRIGGER trigger_sale_item_update
  AFTER UPDATE ON sale_items
  FOR EACH ROW
  EXECUTE FUNCTION handle_sale_item_update();