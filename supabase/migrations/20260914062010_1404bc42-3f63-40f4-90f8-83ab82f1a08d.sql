-- 1) detect_settlement_drift: SECURITY DEFINER + granted to `authenticated` and
--    defaults to ALL organizations. Require platform_admin inside the function body.
...
BEGIN
  ...
  INSERT INTO public.drift_detection_runs (run_at, drifts_found, critical_count, duration_ms, scope_organization_id)
  VALUES (
    now(),
    v_total_drifts,
    v_critical,
...
-- 2) invoice-pdfs storage UPDATE policy: use the same membership subquery as
--    INSERT/SELECT/DELETE instead of a uuid-cast user_belongs_to_org() check.
...
-- 3) website_settings: keep the intentional public storefront read, but make it
--    fail-closed by granting anon SELECT only on the known-public columns, so any
--    future sensitive column is not automatically exposed.
REVOKE SELECT ON public.website_settings FROM anon;
GRANT SELECT (
  organization_id, slug, whatsapp_number, instagram_url,
  facebook_url, theme_accent_color, is_published, custom_domain
