-- Speed up reconcile_customer_balance (called per-customer by
-- get_customer_financial_snapshot_batch). Two changes:
--   1. Add a composite index on sales (organization_id, customer_id) WHERE
--      deleted_at IS NULL so the many EXISTS subqueries can use an index scan
--      instead of touching idx_sales_customer_id without org filter.
--   2. Rewrite reconcile_customer_balance to compare voucher_entries.reference_id
--      (uuid) directly with sales.id (uuid) / customer_id (uuid) rather than
--      casting both sides to text — which prevented use of
--      idx_voucher_entries_org_type_ref and idx_voucher_entries_ref_type_id.
-- Outcome: per-customer reconcile drops from ~80ms to a few ms on large orgs,
-- so get_customer_financial_snapshot_batch no longer hits statement_timeout
-- and the spurious "canceling statement due to statement timeout" toast no
-- longer appears mid-POS bill save.

CREATE INDEX IF NOT EXISTS idx_sales_org_customer_active
