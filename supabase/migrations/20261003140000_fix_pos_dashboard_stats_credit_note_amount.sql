-- Fix get_pos_dashboard_stats: sales.credit_amount does NOT exist.
-- The RPC referenced a nonexistent column and threw at runtime, forcing every
-- POS dashboard load to fall back to the slow client-side aggregation path.
--
-- Sales table has:
--   credit_note_id     (uuid FK → credit_notes.id)
--   credit_note_amount (numeric)   ← this is what the creditNoteCount /
--                                     creditNoteAmount metrics represent
--   credit_applied     (numeric)   ← customer store-credit used as payment
--                                     method; unrelated to CN sales
--
-- Every bare `credit_amount` reference here belongs to the credit-NOTE metric
-- (sale-type = 'cn' filter, with/without_credit_note filter, creditNoteCount /
-- creditNoteAmount aggregates). The client fallback also uses
-- `credit_note_amount` primarily. So the legacy `credit_amount` is replaced
-- with `credit_note_amount` — NOT credit_applied.
