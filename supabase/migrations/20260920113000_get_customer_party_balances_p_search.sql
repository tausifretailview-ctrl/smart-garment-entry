-- Receipt parity: reproduce reconcile_customer_balance receipt_payments exactly (set-based).
-- Canonical rule (20260817120000_fix_advance_double_count_outstanding.sql):
--   voucher_type = receipt, not deleted
--   EXCLUDE: payment_method advance_adjustment OR description adjusted from advance / advance applied to
--   INCLUDE when:
--     (a) EXISTS sale (org, deleted_at IS NULL, s.id::text = ve.reference_id::text) — any reference_type
--     (b) OR reference_type = customer AND trim(ref) = customer_id AND ref is not a sale id
-- Fixes vs prior party RPC:
--   - trim(reference_id) joins (uuid text whitespace)
--   - opening path joins cust on trim(ref)=trim(id) — no ::uuid cast drop
--   - single receipt_voucher_base filter (canonical WHERE), then attribute via org_sales_ref + cust
--
-- Optional p_search: narrow customers before balance aggregation (Customer Balances / Ledger search).

DROP FUNCTION IF EXISTS public._get_customer_party_balances_rows(uuid);

CREATE OR REPLACE FUNCTION public._get_customer_party_balances_rows(
...
AS $$
  ...
  balance_adjustment AS (
    ...
    WHERE cba.organization_id = p_organization_id
      AND cba.customer_id IN (SELECT id FROM cust)
  ...
  pending_sale_returns AS (
    ...
    WHERE sr.organization_id = p_organization_id
      ...
      AND sr.customer_id IN (SELECT id FROM cust)
  ...
  credit_note_vouchers AS (
    ...
    WHERE ve.organization_id = p_organization_id
      ...
      AND ve.reference_id::uuid IN (SELECT id FROM cust)
  ...
  customer_advance_totals AS (
    ...
    WHERE ca.organization_id = p_organization_id
      AND ca.customer_id IN (SELECT id FROM cust)
...
DROP FUNCTION IF EXISTS public.get_customer_party_balances(uuid);

CREATE OR REPLACE FUNCTION public.get_customer_party_balances(
  p_organization_id uuid,
  p_search text DEFAULT NULL
)
RETURNS TABLE (
...
BEGIN
  ...
  FROM public._get_customer_party_balances_rows(p_organization_id, p_search) AS r;
END;
$$;

COMMENT ON FUNCTION public._get_customer_party_balances_rows(uuid, text) IS
  'Internal set-based balance rows. Optional p_search filters customers before aggregation.';

COMMENT ON FUNCTION public.get_customer_party_balances(uuid, text) IS
  'Set-based customer balance list. p_search optional (name/phone ILIKE).';

REVOKE ALL ON FUNCTION public._get_customer_party_balances_rows(uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_customer_party_balances(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._get_customer_party_balances_rows(uuid, text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_customer_party_balances(uuid, text) TO authenticated, service_role;
