-- Phase 1 step 1 (balance SSOT): party RPC CN handling must call
-- public._is_settlement_memo_receipt, not a duplicated LIKE list.
--
-- Why this is isolated from screen migration:
--   14 C-PARTY surfaces (org cards, dashboards, exports, payment picker, AI,
--   Khata FIFO) all read get_customer_party_balances. Fixing the SQL once
--   closes Farhaan-shape CN double-count for every one of them.
--
-- Farhaan Fab (ELLA NOOR, phone 7977353244) — proven fixture:
--   invoiced 17300, gated SRA 2700, cash receipts 14600, CN memo 2700,
--   remaining sale-return credit 100 (partially_adjusted, CAB=100).
--   Canonical / C-JS: -100 Cr.
--   Unpatched (CN memo counted as cash + remaining 100): -2800 Cr.
--   20260911150000 repo body (inline IN-list already drops credit_note_adjustment
--   memos, but pending_sr is credit_status='pending' only): 0, not -100.
--   This migration: helper on receipts + remaining-credit pending_sr → -100.
--
-- Also restores _sale_return_remaining_credit_for_balance (20260822150000 /
-- 20260823160000). 20260911150000 regressed pending_sr to 'pending' only and
-- dropped Farhaan's ₹100 remainder (Hanif bhai / adjusted remainder same hole).
--
-- paid_at_sale_drift receipt subquery matches reconcile_customer_balance
-- (20260823180000): exclude memos so a CN/advance memo cannot hide POS tender.
--
-- Do not fold credit_note_adjustment into receipts. CN consumption is
-- sale_return_adjust. Remaining CN is the leftover pool (like unused advances).
-- Keep items_gross SRA gate. Keep credit_note_vouchers CTE.
--
-- Wrapper get_customer_party_balances (auth + RETURN QUERY) is unchanged.
-- This helper stays LANGUAGE sql STABLE SECURITY DEFINER; org scope is the
-- set-based p_organization_id filter (same as 20260911150000). Do not add
-- IF auth.uid() IS NOT NULL THEN skip-the-check.
...
COMMENT ON FUNCTION public._get_customer_party_balances_rows(uuid) IS
  ...
  'pending_sr is remaining sale-return credit via _sale_return_remaining_credit_for_balance. '
