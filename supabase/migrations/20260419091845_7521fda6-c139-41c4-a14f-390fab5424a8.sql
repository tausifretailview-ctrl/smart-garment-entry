-- Optimize get_product_catalog_page: paginate first, aggregate variants only for the page slice.
-- Previous version aggregated variants for all matching products before LIMIT, causing 8s+ on large catalogs.
...
RETURNS TABLE(
  product_id uuid, product_name text, product_type text, category text, brand text,
  style text, color text, image_url text, hsn_code text, gst_per integer,
  default_pur_price numeric, default_sale_price numeric, status text, size_group_id uuid,
  total_stock bigint, variant_count bigint, total_count bigint
)
LANGUAGE sql
STABLE
...
AS $function$
  ...
  -- Fast path: no stock/price filters → paginate directly on products, aggregate variants only for the page slice.
  fast_page as (
    select b.*
    from base b
  ...
  fast_with_variants as (
    select fp.*,
           coalesce((select sum(coalesce(pv.stock_qty,0))::bigint
                     from public.product_variants pv
                     where pv.product_id = fp.id
                       and pv.organization_id = p_org_id
                       and pv.deleted_at is null), 0)::bigint as total_stock,
           coalesce((select count(*)::bigint
                     from public.product_variants pv
                     where pv.product_id = fp.id
                       and pv.organization_id = p_org_id
                       and pv.deleted_at is null), 0)::bigint as variant_count,
           (select c from fast_total) as total_count
    from fast_page fp
  ),
  -- Slow path: stock/price filters require aggregating across all matching products.
  slow_totals as (
  ...
  slow_page as (
    ...
    order by sq.created_at desc nulls last, sq.product_id
    offset greatest((greatest(coalesce(p_page,1),1) - 1) * greatest(coalesce(p_page_size,50),1), 0)
    limit greatest(coalesce(p_page_size,50), 1)
  )
  select id, product_name, product_type, category, brand, style, color, image_url,
         hsn_code, gst_per, default_pur_price, default_sale_price, status, size_group_id,
         total_stock, variant_count, total_count
