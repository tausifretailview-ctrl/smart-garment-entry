...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN agg.atype IN ('Asset', 'Expense') THEN GREATEST(agg.md - agg.mc, 0::numeric)
      ELSE GREATEST(agg.mc - agg.md, 0::numeric)
    END AS trial_debit,
    CASE
      WHEN agg.atype IN ('Asset', 'Expense') THEN GREATEST(agg.mc - agg.md, 0::numeric)
      ELSE GREATEST(agg.md - agg.mc, 0::numeric)
    END AS trial_credit
  FROM agg
  ORDER BY agg.acode;
END;
