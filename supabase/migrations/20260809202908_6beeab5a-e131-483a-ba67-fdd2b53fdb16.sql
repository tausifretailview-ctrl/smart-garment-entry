DROP VIEW IF EXISTS public.v_accounting_invariants;

CREATE VIEW public.v_accounting_invariants
...
UNION ALL
 -- NEW: applications tagged to a booking (plus its refunds) must not exceed the booking amount.
   ...
   FROM customer_advances a
     JOIN LATERAL ( SELECT COALESCE(sum(ve.total_amount), 0::numeric) AS applied
          ...
          WHERE ve.organization_id = a.organization_id AND ve.deleted_at IS NULL
            ...
            AND ve.description LIKE '%' || a.advance_number || '%') x ON true
...
UNION ALL
 -- NEW: a customer's total advance draws (applications + refunds) must not exceed advances received.
 SELECT 'customer_advance_pool_negative', t.organization_id, t.customer_id, t.customer_name,
    round(t.drawn - t.received, 2)
   FROM (
     SELECT c.organization_id, c.id AS customer_id, c.customer_name AS customer_name,
