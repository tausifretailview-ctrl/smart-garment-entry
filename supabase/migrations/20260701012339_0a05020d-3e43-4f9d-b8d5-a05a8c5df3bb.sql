...
BEGIN
  ...
  reconciled AS (
    SELECT
      ...
      -- Use GREATEST(sale_return_adjust, credit_applied) so legacy rows where
      -- credit was applied via the deprecated `credit_applied` column (no live
      -- voucher) are still subtracted from outstanding. Rows where both equal
      -- (canonical) are unaffected.
      public.invoice_reconcile_outstanding(
        COALESCE(fs.net_amount, 0),
        GREATEST(COALESCE(fs.sale_return_adjust, 0), COALESCE(fs.credit_applied, 0)),
        GREATEST(
          0,
      ...
      CASE
        ...
        WHEN COALESCE(rs.cash, 0) > 0.01
          ...
          OR COALESCE(fs.credit_applied, 0) > 0.01
          THEN 'partial'::text
        ELSE 'pending'::text
