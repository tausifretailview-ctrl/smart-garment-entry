-- Purchase Bill Dashboard: sort by bill entry date & time (EzzyERP save time),
-- not supplier bill_date. Falls back to created_at for older rows.
...
COMMENT ON FUNCTION public.get_purchase_bills_dashboard_page(
  ...
  'Purchase bill dashboard page. Sort uses COALESCE(bill_entry_at, created_at) (EzzyERP entry time).';
