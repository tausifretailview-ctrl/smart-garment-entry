-- Shumama / CN-customer fix: party RPC was double-counting credit_note_adjustment receipts.
-- Those amounts are already netted via sales.sale_return_adjust (canonical excludes them).
-- Regression: 20260909120400 receipt_parity widened advance exclusion but omitted CN exclusion
-- that 20260623203800 had added. Restores parity with reconcile_customer_balance receipt_payments.
...
COMMENT ON FUNCTION public._get_customer_party_balances_rows(uuid) IS
  'Set-based party balances. receipt_payments excludes advance + CN adjustment memos (parity with reconcile_customer_balance).';
