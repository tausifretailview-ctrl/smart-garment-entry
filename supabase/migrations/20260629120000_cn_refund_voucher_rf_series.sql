-- CN / sale-return cash refunds: RF/YY-YY/N series (financial year), stored as voucher_entries payment rows.
...
BEGIN
  ...
  IF p_type = 'cn_refund' THEN
    SELECT COALESCE(
      MAX(CAST(SUBSTRING(voucher_number FROM 'RF/\d+-\d+/(\d+)$') AS INTEGER)),
      0
    ) + 1
    INTO v_count
    FROM public.voucher_entries
    WHERE voucher_number LIKE 'RF/' || financial_year || '/%'
...
COMMENT ON FUNCTION public.generate_voucher_number(text, date) IS
  'FY voucher numbers: RCP, PAY, RF (cn_refund / sale-return CN cash refund), etc.';
