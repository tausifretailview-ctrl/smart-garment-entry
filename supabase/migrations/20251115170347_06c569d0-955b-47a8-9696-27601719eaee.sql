...
-- Allow admins and managers to view all logs
...
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    PERFORM log_audit(
      ...
      NULL,
      jsonb_build_object('table', 'sales', 'warning', 'Sale deletion affects stock levels')
    );
  END IF;
...
-- Create trigger function for purchase bills changes
...
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    PERFORM log_audit(
      ...
      NULL,
      jsonb_build_object('table', 'purchase_bills', 'warning', 'Purchase deletion affects stock levels')
    );
  END IF;
...
-- Create trigger for purchase_bills table
...
-- Create trigger function for stock movement tracking
...
-- Create trigger for stock_movements table
