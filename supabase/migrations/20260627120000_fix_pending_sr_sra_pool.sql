-- Pending SR credit in reconcile: absorb against invoice sale_return_adjust pool
-- when CN was applied on a bill but the return row is still `pending` without linked_sale_id.
...
COMMENT ON FUNCTION public.reconcile_customer_balance(uuid, uuid) IS
  'Lifetime outstanding components. Pending SR net of linked invoice S/R and unlinked pool vs total sale_return_adjust.';
