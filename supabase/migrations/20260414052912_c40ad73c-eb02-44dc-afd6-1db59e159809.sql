...
-- Drop dependent function first, then the base function
DROP FUNCTION IF EXISTS public.fix_stock_discrepancies(uuid);
DROP FUNCTION IF EXISTS public.detect_stock_discrepancies(uuid);
DROP FUNCTION IF EXISTS public.reset_stock_from_transactions(uuid);
DROP FUNCTION IF EXISTS public.reconcile_variant_stock_qty(uuid);

-- Recreate detect_stock_discrepancies with transaction-based formula
...
-- Recreate fix_stock_discrepancies
CREATE OR REPLACE FUNCTION public.fix_stock_discrepancies(p_organization_id uuid)
...
BEGIN
  ...
  LOOP
    ...
    SELECT v_record.variant_id, 'reconciliation', 0,
      'Reconciliation: adjusted from ' || v_record.current_stock_qty || ' to ' || v_record.calculated_stock_qty || ' (diff: ' || (-v_record.discrepancy) || ')',
...
-- Recreate reset_stock_from_transactions with pending challans
...
-- Recreate reconcile_variant_stock_qty with transaction-based formula
