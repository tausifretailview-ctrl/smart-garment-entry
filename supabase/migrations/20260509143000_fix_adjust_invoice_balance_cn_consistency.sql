...
BEGIN
    SELECT net_amount, paid_amount, credit_applied, sale_return_adjust, payment_status
    INTO v_net_amount, v_paid_amount, v_credit_applied, v_sale_return_adjust, v_current_status
    FROM sales
    ...
    -- CN adjustments in app flows are represented through sale_return_adjust.
    -- Keep validation aligned with UI pending balance to avoid false "invoice balance 0" errors.
    v_invoice_balance := COALESCE(v_net_amount, 0) - COALESCE(v_paid_amount, 0) - COALESCE(v_sale_return_adjust, 0);
    ...
    v_new_sr_adjust := COALESCE(v_sale_return_adjust, 0) + p_amount_applied;
    v_remaining := COALESCE(v_net_amount, 0) - (COALESCE(v_paid_amount, 0) + v_new_sr_adjust);
    ...
    SET
        ...
        -- Keep legacy field synchronized for older reads.
        credit_applied = v_new_sr_adjust,
