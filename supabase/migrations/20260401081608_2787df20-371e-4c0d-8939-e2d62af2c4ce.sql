...
CREATE OR REPLACE FUNCTION generate_fee_receipt_number(p_organization_id UUID, p_fy_start_year INTEGER DEFAULT NULL, p_fy_end_year INTEGER DEFAULT NULL)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- Read configurable FY start month from settings (default 4 = April)
  SELECT (sale_settings::jsonb)->>'fee_fy_start_month'
  INTO v_settings
  FROM settings
  WHERE organization_id = p_organization_id;

  fy_start_month := COALESCE(v_settings::INTEGER, 4);

  -- Use provided year params or calculate from current date
