...
-- 1. Tracking columns on customer_balance_adjustments
ALTER TABLE public.customer_balance_adjustments
  ADD COLUMN IF NOT EXISTS materialized_at timestamptz,
  ADD COLUMN IF NOT EXISTS materialized_by uuid,
  ADD COLUMN IF NOT EXISTS materialized_voucher_ids uuid[];

CREATE INDEX IF NOT EXISTS idx_cba_org_customer_unmaterialized
  ON public.customer_balance_adjustments (organization_id, customer_id)
  WHERE materialized_at IS NULL AND outstanding_difference < 0;

-- 2. Detection function: list customers whose per-invoice pending totals
-- disagree with the customer's authoritative ledger closing balance.
...
AS $fn$
BEGIN
  IF auth.uid() IS NOT NULL AND NOT public.has_org_role(auth.uid(), p_organization_id, ARRAY['admin','owner','platform_admin']) THEN
  ...
  WITH ledger AS (
    SELECT c.id AS cid, c.name AS cname,
           COALESCE((
             SELECT SUM(amount) FROM public.reconcile_customer_balance_v2(c.id, p_organization_id)
           ), 0)::numeric AS closing
  ...
  floating AS (
    SELECT cba.customer_id,
           SUM(cba.outstanding_difference)::numeric AS pool
...
REVOKE ALL ON FUNCTION public.detect_balance_adjustment_drift(uuid, numeric) FROM public;
GRANT EXECUTE ON FUNCTION public.detect_balance_adjustment_drift(uuid, numeric) TO authenticated, service_role;

-- 3. Repair RPC: convert floating reduce-adjustments into receipt vouchers,
--    allocated FIFO (Opening Balance first, then oldest unpaid invoices).
--    Balance-preserving: aborts if closing balance moves > 1 rupee.
...
BEGIN
  ...
  -- Snapshot closing balance BEFORE any change
  ...
  -- Sum floating reduce-adjustments (only negative outstanding_difference = money in)
  ...
  -- ── Step 1: Allocate to Opening Balance (as "customer" reference) ──
  ...
  IF COALESCE(v_op_pending,0) > 0 AND v_remaining > 0 THEN
    ...
    reference_id := p_customer_id;
    reference_label := 'Opening Balance';
    amount := v_alloc;
    voucher_id := v_voucher_id;
    RETURN NEXT;
  END IF;

  -- ── Step 2: Allocate FIFO to oldest unpaid invoices ──
  FOR v_target IN
  ...
  LOOP
    ...
    action := 'allocate';
    reference_type := 'sale';
    reference_id := v_target.id;
    reference_label := v_target.sale_number;
    amount := v_alloc;
    voucher_id := v_voucher_id;
    RETURN NEXT;
  END LOOP;

  -- If we could not allocate the full pool, emit a "residual" row so caller sees it.
  IF v_remaining > 0.01 THEN
    action := 'residual_unallocated';
    reference_type := NULL;
    reference_id := NULL;
  ...
  -- ── Step 3: Mark source adjustments as materialized ──
  IF NOT p_dry_run THEN
    ...
    -- ── Step 4: Balance-preservation guard ──
    ...
    IF ABS(v_after_closing - v_before_closing) > 1.0 THEN
      RAISE EXCEPTION 'Repair aborted: closing balance would change from % to %. Rolling back.',
        v_before_closing, v_after_closing
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
END;
...
REVOKE ALL ON FUNCTION public.repair_customer_floating_adjustments(uuid, uuid, boolean) FROM public;
