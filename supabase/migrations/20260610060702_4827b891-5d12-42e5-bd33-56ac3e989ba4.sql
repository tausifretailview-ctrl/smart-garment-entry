...
BEGIN
  ...
  -- Total paid = at-sale tender (cash/card/UPI columns) + post-sale customer receipts, capped at net.
  new_paid := LEAST(v_payable_cap, GREATEST(0, COALESCE(v_tender, 0)) + v_receipt_total);
...
WITH recompute AS (
  SELECT s.id, c.new_paid, c.new_status
  FROM public.sales s
