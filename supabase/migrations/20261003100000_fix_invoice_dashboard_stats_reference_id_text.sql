-- Fix get_invoice_dashboard_stats: voucher_entries.reference_id is uuid;
-- comparisons against ::text literals caused "operator does not exist: uuid = text".
...
BEGIN
    ...
    WHERE ve.organization_id = p_organization_id
      ...
      AND (
        (
          ...
          AND (
            ve.reference_id::text IN (SELECT fs.id::text FROM filtered_sales fs)
            OR ve.reference_id::text IN (
        ...
        OR (
          ...
          AND EXISTS (
            SELECT 1
            FROM filtered_sales fs
            WHERE ve.reference_id::text = fs.id::text
              OR (
                fs.customer_id IS NOT NULL
                AND ve.reference_id::text = fs.customer_id::text
              )
              OR (
  ...
  receipt_attributed AS (
    ...
    INNER JOIN receipt_rows rr ON (
      rr.reference_id::text = fs.id::text
      OR (
        fs.customer_id IS NOT NULL
        AND rr.reference_id::text = fs.customer_id::text
