...
-- Permanent guardrail: ensure save_purchase_bill_with_items_atomic ALWAYS applies
-- stock effects when it sets app.bulk_purchase_insert='1'. If a future edit
-- forgets to call _apply_bulk_purchase_insert_effects, this assertion will
-- raise instead of saving a bill with stock_qty = 0.
...
BEGIN
  ...
  -- Apply stock / batch / movement effects skipped by the per-row trigger.
  PERFORM public._apply_bulk_purchase_insert_effects(v_bill_id);

  PERFORM set_config('app.bulk_purchase_insert', '', true);

  -- ===== PERMANENT GUARDRAIL =====
  -- Verify stock_movements were actually written for every purchased qty.
  -- If a future edit forgets to call _apply_bulk_purchase_insert_effects,
  -- this assertion raises and the whole transaction rolls back, instead of
  -- silently leaving stock_qty = 0 (the 2026-06-19 Ella Noor incident).
  SELECT COALESCE(SUM(qty), 0) INTO v_expected_qty
  FROM public.purchase_items
  WHERE bill_id = v_bill_id AND deleted_at IS NULL;

  SELECT COALESCE(SUM(quantity), 0) INTO v_actual_movements
  FROM public.stock_movements
  WHERE reference_id = v_bill_id AND movement_type = 'purchase';
...
COMMENT ON FUNCTION public.save_purchase_bill_with_items_atomic(uuid, jsonb, jsonb) IS
  'Atomic purchase bill save. MUST call _apply_bulk_purchase_insert_effects after the bulk items INSERT (per-row trigger is skipped via app.bulk_purchase_insert=1). The post-save assertion comparing SUM(stock_movements.quantity) to SUM(purchase_items.qty) is a guardrail against the 2026-06-19 regression and must NOT be removed.';
