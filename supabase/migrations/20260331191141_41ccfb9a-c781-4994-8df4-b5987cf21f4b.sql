-- Fix: Use IST (Asia/Kolkata) instead of UTC for financial year calculation
-- This ensures FY rolls over at midnight IST (April 1st) not midnight UTC
...
BEGIN
  ...
  current_month := EXTRACT(MONTH FROM ist_date);
  current_year  := EXTRACT(YEAR  FROM ist_date);
...
DECLARE v_count INTEGER; v_number TEXT; financial_year TEXT; ist_date DATE;
  fy_start_year INTEGER; fy_end_year INTEGER;
BEGIN
  ist_date := (now() AT TIME ZONE 'Asia/Kolkata')::date;
  IF EXTRACT(MONTH FROM ist_date) >= 4 THEN
    fy_start_year := EXTRACT(YEAR FROM ist_date); fy_end_year := fy_start_year + 1;
  ELSE
    fy_end_year := EXTRACT(YEAR FROM ist_date); fy_start_year := fy_end_year - 1;
  END IF;
  financial_year := SUBSTRING(fy_start_year::TEXT FROM 3 FOR 2) || '-' || SUBSTRING(fy_end_year::TEXT FROM 3 FOR 2);
  SELECT COALESCE(MAX(CAST(SUBSTRING(sale_number FROM 'POS/\d+-\d+/(\d+)$') AS INTEGER)), 0) + 1
  INTO v_count FROM sales
  WHERE organization_id = p_organization_id AND sale_number LIKE 'POS/' || financial_year || '/%' AND deleted_at IS NULL;
...
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_prefix TEXT := 'PUR'; v_count INTEGER; v_number TEXT; financial_year TEXT; ist_date DATE;
  fy_start_year INTEGER; fy_end_year INTEGER;
BEGIN
  ist_date := (now() AT TIME ZONE 'Asia/Kolkata')::date;
  IF EXTRACT(MONTH FROM ist_date) >= 4 THEN
  ...
  INTO v_count FROM purchase_bills
  WHERE organization_id = p_organization_id AND software_bill_no LIKE v_prefix || '/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM sale_returns
  WHERE organization_id = p_organization_id AND return_number LIKE 'SR/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM sale_orders
  WHERE organization_id = p_organization_id AND order_number LIKE 'SO/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM purchase_returns
  WHERE organization_id = p_organization_id AND return_number LIKE 'PR/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM delivery_challans
  WHERE organization_id = p_organization_id AND challan_number LIKE 'DC/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM quotations
  WHERE organization_id = p_organization_id AND quotation_number LIKE 'QT/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM credit_notes
  WHERE organization_id = p_organization_id AND credit_note_number LIKE 'CN/' || financial_year || '/%' AND deleted_at IS NULL;
...
BEGIN
  ...
  INTO v_count FROM customer_advances
  WHERE organization_id = p_organization_id AND advance_number LIKE 'ADV/' || financial_year || '/%';
...
BEGIN
  ...
  END IF;
  financial_year := SUBSTRING(fy_start_year::TEXT FROM 3 FOR 2) || '-' || SUBSTRING(fy_end_year::TEXT FROM 3 FOR 2);
  SELECT COALESCE(MAX(CAST(SUBSTRING(voucher_number FROM 'RCT/\d+-\d+/(\d+)$') AS INTEGER)), 0) + 1
  INTO v_count FROM voucher_entries
  WHERE organization_id = p_organization_id AND voucher_number LIKE 'RCT/' || financial_year || '/%';
...
BEGIN
  ...
  END IF;
  financial_year := SUBSTRING(fy_start_year::TEXT FROM 3 FOR 2) || '-' || SUBSTRING(fy_end_year::TEXT FROM 3 FOR 2);
  SELECT COALESCE(MAX(CAST(SUBSTRING(order_number FROM 'PO/\d+-\d+/(\d+)$') AS INTEGER)), 0) + 1
  INTO v_count FROM purchase_orders
  WHERE organization_id = p_organization_id AND order_number LIKE 'PO/' || financial_year || '/%' AND deleted_at IS NULL;
  v_number := 'PO/' || financial_year || '/' || v_count::TEXT;
  RETURN v_number;
END; $$;