-- Align org-level reconcile_customer_balances with get_customer_true_outstanding
-- (Customer Ledger / financial snapshot). Fixes inflated total_invoices and
-- subtracting full advance received instead of lifetime outstanding components.
...
COMMENT ON FUNCTION public.reconcile_customer_balances(UUID) IS
  'Org customer balances: calculated_balance = get_customer_true_outstanding (ledger-aligned). '
  'total_invoices = gated gross; total_advance_used = SUM(used_amount).';
