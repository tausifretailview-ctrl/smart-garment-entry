...
COMMENT ON FUNCTION public.get_product_wise_stock_filter_options(uuid) IS
  'Distinct brand/category/department/supplier values for Item-wise Stock Report filters.';

COMMENT ON FUNCTION public.get_product_wise_stock_report_totals(uuid, text, text, text, text, text, text) IS
  'Grand totals for Item-wise Stock Report (same filters as get_product_wise_stock_report).';

COMMENT ON FUNCTION public.get_product_wise_stock_report(uuid, integer, integer, text, text, text, text, text, text) IS
  'Paginated Item-wise Stock Report rows aggregated by product_name, supplier, brand, category, or department.';
