...
AS $function$
  ...
  sale_components AS (
    SELECT s.customer_id,
      -- invoiced (net + legacy sr_adjust gated)
      s.net_amount
        + CASE
          ...
          END AS invoiced,
      -- receipt payments matched to this sale (excl advance adjustment)
      COALESCE(rps.recd, 0) AS receipts_on_sale,
      -- POS pay-at-sale drift (>= 0)
      GREATEST(
        0::numeric,
  ...
  open_recv AS (
    -- receipts attached to a customer (not to a sale)
