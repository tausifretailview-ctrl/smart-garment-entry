-- Custom POS / INV number allocation: ignore soft-deleted sales when finding MAX
-- and when checking uniqueness. Previously deleted trial invoices still blocked
-- reuse (preview showed POS/26-27/4 after recycle-bin purge). Also derive the
-- LIKE pattern from the format prefix (POS/26-27/{###} → POS/26-27/%) instead
-- of a bare calendar-year wildcard that never matched POS/26-27/N numbers.
...
BEGIN
  ...
  v_like_pattern := regexp_replace(p_format, '\{#+\}', '%');
