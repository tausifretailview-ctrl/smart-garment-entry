...
CREATE OR REPLACE FUNCTION public.repair_customer_floating_adjustments(
  p_organization_id uuid, p_customer_id uuid, p_dry_run boolean DEFAULT true
)
RETURNS TABLE(
  action text, reference_type text, reference_id uuid,
  reference_label text, amount numeric, voucher_id uuid
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $fn$
DECLARE
  v_before_closing numeric; v_after_closing numeric;
  v_pool numeric; v_remaining numeric; v_alloc numeric;
  v_voucher_id uuid; v_adj_ids uuid[]; v_target record;
  v_op_pending numeric; v_actor uuid; v_first_adj_date date;
BEGIN
  IF auth.uid() IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.organization_members om
    WHERE om.user_id = auth.uid() AND om.organization_id = p_organization_id
