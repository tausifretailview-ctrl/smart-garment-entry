...
AS $function$
  ...
  -- Per-sale voucher receipts (excluding advance-funded ones).
  -- Classify by id-match (handles legacy reference_type='customer' with reference_id=sale_id).
  sale_vouch AS (
    SELECT ve.reference_id AS sale_id, SUM(ve.total_amount) AS voucher_total
  ...
  -- Cash payments per sale: GREATEST of recorded paid_amount vs voucher sum.
  -- This handles two drift cases:
  --   (a) paid_amount set on POS but voucher never written
  --   (b) vouchers exist but paid_amount stale (older value)
  -- Subtract sale_return_adjust because the offsetting CN-adjustment voucher is excluded
  -- from sale_vouch (CN adjustments use payment_method='credit_note_adjustment' which IS
  -- counted) — actually CN adjustments ARE in sale_vouch; the GREATEST handles drift safely.
  cash_pay AS (
    SELECT s.customer_id, 
      SUM(GREATEST(COALESCE(s.paid_amount, 0), COALESCE(sv.voucher_total, 0))) AS total
    FROM sales s
  ...
  -- Opening balance payments: only receipts that genuinely point to a customer (not a sale).
  open_pay AS (
