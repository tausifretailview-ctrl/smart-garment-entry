...
-- Drop the old 1-param overload that causes ambiguity
DROP FUNCTION IF EXISTS public.generate_fee_receipt_number(uuid);

-- Create a read-only peek function for previewing next receipt number
...
BEGIN
  ...
  -- Just read the current sequence without incrementing
