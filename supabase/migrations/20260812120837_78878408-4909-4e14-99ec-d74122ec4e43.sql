CREATE OR REPLACE FUNCTION public._get_supplier_party_balances_rows(p_organization_id uuid)
 RETURNS TABLE(out_supplier_id uuid, out_supplier_name text, out_signed_balance numeric, out_direction text, out_total_cr numeric, out_total_dr numeric, out_net_payable numeric)
 LANGUAGE sql
...
AS $function$
  ...
  bill_return_adjust AS (
    -- Purchase-return credit written straight into purchase_bills.paid_amount
    -- (Adjust CN -> bill) with NO credit-note voucher behind it. Such returns are
    -- already subtracted once via unreflected_returns, so they must not also count
    -- as cash paid on the bill.
    SELECT
      pr.linked_bill_id AS bill_id,
      ROUND(COALESCE(SUM(GREATEST(0::numeric, COALESCE(pr.net_amount, 0))), 0)::numeric, 2) AS amt
    ...
    WHERE pr.organization_id = p_organization_id
      ...
      AND (
        ...
        OR NOT EXISTS (
          SELECT 1
          FROM public.voucher_entries cn
          WHERE cn.id = pr.credit_note_id
            AND cn.organization_id = p_organization_id
            AND cn.deleted_at IS NULL
            AND lower(COALESCE(cn.voucher_type, '')) = 'credit_note'
            AND lower(COALESCE(cn.reference_type, '')) IN (
              'supplier', 'supplierpayment', 'supplier_payment', 'purchase'
            )
        )
      )
    ...
    SELECT
      ...
      ROUND(COALESCE(SUM(
        CASE
          ...
          ELSE GREATEST(0::numeric, ob.paid_amount - COALESCE(bra.amt, 0))
        END
      ), 0)::numeric, 2) AS amt
    FROM org_bills ob
    LEFT JOIN per_bill_voucher_paid pbv ON pbv.bill_id = ob.id
    LEFT JOIN bill_return_adjust bra ON bra.bill_id = ob.id
    GROUP BY ob.supplier_id
  ),
