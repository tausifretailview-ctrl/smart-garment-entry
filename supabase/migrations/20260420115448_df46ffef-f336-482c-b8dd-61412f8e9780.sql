...
AS $function$
  ...
  -- Cash payments: include all non-advance receipts (credit_note_adjustment included
  -- because sale_returns are subtracted separately and CN application offsets them).
  ...
  SELECT 
    ...
    -- Balance = Opening + Invoices - CashPayments - AdvanceUsed - SaleReturns + Refunds + Adjustments
    (c.opening_balance 
     + COALESCE(i.total, 0) 
