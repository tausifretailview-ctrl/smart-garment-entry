-- Speed up get_invoice_dashboard_stats (ELLA NOOR / statement_timeout 57014).
--
-- Hot path was O(receipts × invoices): description ILIKE '%'||sale_number||'%' against
-- every org receipt when the dashboard has no search/customer narrow scope, plus
-- reference_id::text casts that blocked uuid indexes, plus invoice_reconcile_outstanding
-- evaluated twice per row.
--
-- Keep sale-id and customer-id uuid joins; keep description match only for
-- customer-scoped receipts already filtered to customers in the date window.
...
BEGIN
  ...
  items_gross AS (
    ...
    FROM sale_items si
    INNER JOIN sale_ids sid ON sid.id = si.sale_id
