-- Align get_supplier_party_balances paid/CN with computeSnapshotForSupplier:
--   paid = max(bill-linked vouchers, paid_amount minus CN/return applied to the bill)
--     plus supplier-id vouchers only for the slice not already in bill residual
--   (no description substring matching).
--
...
AS $$
  ...
  return_adjust_by_bill AS (
    SELECT
      pr.linked_bill_id AS bill_id,
      COALESCE(SUM(
        CASE
          WHEN lower(trim(COALESCE(pr.credit_status, ''))) IN ('adjusted', 'partially_adjusted')
            THEN
              CASE
                WHEN pr.credit_available_balance IS NULL THEN GREATEST(0::numeric, COALESCE(pr.net_amount, 0))
                ELSE GREATEST(0::numeric, COALESCE(pr.net_amount, 0) - COALESCE(pr.credit_available_balance, 0))
              END
          ELSE 0::numeric
        END
      ), 0)::numeric AS adj
    ...
    GROUP BY pr.linked_bill_id
  ),
  bill_settled_by_supplier AS (
    SELECT
      ob.supplier_id,
      ROUND(COALESCE(SUM(
        GREATEST(
    ...
    LEFT JOIN return_adjust_by_bill ra ON ra.bill_id = ob.id
    GROUP BY ob.supplier_id
  ),
  ...
  total_paid AS (
    SELECT
      COALESCE(bs.supplier_id, slp.supplier_id) AS supplier_id,
      ROUND((
    ...
    FROM bill_settled_by_supplier bs
    FULL OUTER JOIN supplier_level_payments slp ON slp.supplier_id = bs.supplier_id
  ),
