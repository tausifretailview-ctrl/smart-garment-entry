-- fix_detect_settlement_drift_phantom_credit_guard
...
BEGIN
  ...
  classified AS (
    SELECT
      ...
      -- Credit applied on the bill that is backed neither by a credit/advance/balance
      -- adjustment receipt nor by the bill's own sale-return adjustment.
      GREATEST(0::numeric, c.credit_applied - GREATEST(0::numeric, c.sale_return_adjust)) AS unbacked_credit,
      (c.net_amount - c.recorded_paid - c.sale_return_adjust) AS residual,
      CASE
        WHEN c.credit_applied > 0.01
             AND NOT c.has_credit_backing
             AND (c.credit_applied - GREATEST(0::numeric, c.sale_return_adjust)) > 0.01
          THEN 'PHANTOM_CREDIT'
  ...
  drifting AS (
    SELECT
      ...
      CASE
        WHEN drift_type = 'PHANTOM_CREDIT' THEN unbacked_credit
        ELSE (recorded_paid - voucher_paid)
      END AS drift_amount,
