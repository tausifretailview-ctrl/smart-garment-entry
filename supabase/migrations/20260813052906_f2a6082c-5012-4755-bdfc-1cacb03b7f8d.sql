...
UNION ALL
 SELECT 'rapid_duplicate_receipt'::text AS check_name,
   ...
   FROM public.voucher_entries a
     JOIN public.voucher_entries b ON b.organization_id = a.organization_id AND b.reference_id = a.reference_id AND b.total_amount = a.total_amount AND b.id > a.id AND b.created_at >= a.created_at AND b.created_at <= (a.created_at + '00:05:00'::interval)
...
UNION ALL
 SELECT 'advance_refund_exceeds_available'::text AS check_name,
   ...
   FROM public.customer_advances a
     JOIN public.advance_refunds r ON r.advance_id = a.id
...
UNION ALL
 SELECT 'advance_applied_exceeds_invoice'::text AS check_name,
   ...
   FROM public.sales s
     JOIN public.voucher_entries ve ON ve.reference_id = s.id AND ve.voucher_type = 'receipt'::text AND ve.payment_method = 'advance_adjustment'::text AND ve.deleted_at IS NULL
...
UNION ALL
 SELECT 'paid_exceeds_net'::text AS check_name,
   ...
   FROM public.sales s
  WHERE s.deleted_at IS NULL AND COALESCE(s.is_cancelled, false) = false AND s.net_amount > 0 AND COALESCE(s.paid_amount, 0::numeric) > (s.net_amount + COALESCE(s.sale_return_adjust, 0::numeric) + 1::numeric)
UNION ALL
