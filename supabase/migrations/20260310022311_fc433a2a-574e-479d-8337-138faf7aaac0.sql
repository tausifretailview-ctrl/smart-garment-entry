...
-- Cancel invoice RPC (atomic: deletes items → trigger restores stock → marks cancelled)
...
BEGIN
  ...
  -- Delete sale_items: the existing handle_sale_item_delete trigger
  -- automatically restores stock_qty for each variant
  DELETE FROM sale_items WHERE sale_id = p_sale_id;

  -- Mark cancelled — deleted_at stays NULL so it remains visible in history
  UPDATE sales SET
    is_cancelled     = true,
    cancelled_at     = NOW(),
  ...
  -- Nullify any receipt vouchers linked to this invoice
  UPDATE voucher_entries
  SET reference_type = 'cancelled_invoice',
      notes = COALESCE(notes, '') || ' [Invoice Cancelled]',
      updated_at = NOW()
