/**
 * FROZEN copy of CustomerLedger.tsx retail queryFn for dual-run verification.
 * NOT imported by the Vite app. Do not use in product code.
 */
...
export async function fetchCustomerLedgerTransactionsDesktopInline(
  supabase: SupabaseClient,
  organizationId: string,
  selectedCustomer: { id: string; opening_balance: number },
  startDate?: Date | null,
  endDate?: Date | null,
  ...
  const cnAdjustClockBySaleId: Record<string, { date: string; timestamp: string | null }> = {};
  (vouchersData || []).forEach((v: any) => {
    ...
    const createdAt = v.created_at ? String(v.created_at) : null;
    const prev = cnAdjustClockBySaleId[v.reference_id];
    const prevKey = prev ? `${prev.date}\t${prev.timestamp || ""}` : "";
    const nextKey = `${voucherDate}\t${createdAt || ""}`;
    if (!prev || nextKey > prevKey) {
      cnAdjustClockBySaleId[v.reference_id] = {
        date: voucherDate || String(v.voucher_date || ""),
        timestamp: createdAt,
      };
    }
  });
  ...
  combined.forEach((item) => {
    if (item.type === 'invoice') {
      ...
      if (!isCancelled && saleReturnAdjust > 0) {
        const cnClock = cnAdjustClockBySaleId[sale.id];
        allTransactions.push({
          id: `${sale.id}-cn-applied`,
          date: cnClock?.date || sale.sale_date,
          timestamp: cnClock?.timestamp || item.timestamp || null,
          type: 'cn_adjusted',
          reference: sale.sale_number,
      ...
      // Prefer FIFO-allocated CN amounts (Maseera: two SRs sharing one invoice SRA).
      // Fall back to linked-invoice SRA only when that invoice has no CN receipt
      // (billing absorb / SHAHIN). If CN receipts exist but FIFO allocated them
      // to a sibling SR, consume 0 so the leftover row keeps its remaining.
      const consumedAmount = saleReturnConsumedForRemaining({
        ...
        linkedSaleCnVoucherTotal: linkedSaleId
          ? cnVoucherBySaleId[linkedSaleId] || 0
          : 0,
      });
      ...
      // Tracing memo instead of omitting adjusted+linked SRs (SR/159 class).
