...
async function pagedSelect(table, cols, apply) {
  const out = [];
  let from = 0;
  for (;;) {
...
async function fetchCustomersWithFinancialActivity(organizationId) {
  ...
  const [
    ...
    pagedSelect("sales", "customer_id", (q) => q.eq("organization_id", organizationId).is("deleted_at", null).not("customer_id", "is", null)),
    pagedSelect("customer_advances", "customer_id", (q) => q.eq("organization_id", organizationId).not("customer_id", "is", null)),
    pagedSelect("sale_returns", "customer_id", (q) => q.eq("organization_id", organizationId).is("deleted_at", null).not("customer_id", "is", null)),
    pagedSelect("customer_balance_adjustments", "customer_id", (q) => q.eq("organization_id", organizationId).not("customer_id", "is", null)),
    pagedSelect("voucher_entries", "reference_id", (q) => q.eq("organization_id", organizationId).eq("reference_type", "customer").not("reference_id", "is", null)),
    pagedSelect("customers", "id", (q) => q.eq("organization_id", organizationId).is("deleted_at", null).neq("opening_balance", 0)),
  ]);
...
async function fetchBatchMap(organizationId, customerIds) {
  ...
  for (let i = 0; i < customerIds.length; i += BATCH_CHUNK) {
    ...
    let data = null;
    let error = null;
    ({ data, error } = await supabase.rpc("get_customer_financial_snapshot_batch", {
      p_organization_id: organizationId,
      p_customer_ids: chunk,
    }));
    if (error) {
      // fall back to one-by-one so a single slow customer cannot kill the run
      data = [];
      for (const cid of chunk) {
        let ok = false;
        for (let attempt = 0; attempt < 3 && !ok; attempt++) {
          const r = await supabase.rpc("get_customer_financial_snapshot_batch", {
            p_organization_id: organizationId,
            p_customer_ids: [cid],
          });
          if (!r.error) { data.push(...(r.data || [])); ok = true; }
        }
        if (!ok) console.error(`  !! batch timeout for customer ${cid}`);
      }
    }
...
async function fetchAllMap(organizationId) {
  ...
  for (;;) {
    const { data, error } = await supabase
      .rpc("get_customer_financial_snapshot_all", { p_organization_id: organizationId })
      .range(from, from + 999);
    if (error) throw error;
    rows.push(...(data || []));
    if (!data || data.length < 1000) break;
...
function compareMaps(batchMap, allMap, label) {
  ...
  if (diffs.length > 500) {
    console.error(`  … and ${diffs.length - 500} more`);
  }
  return diffs.length;
}
