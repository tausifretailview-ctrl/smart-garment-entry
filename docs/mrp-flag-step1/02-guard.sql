-- STEP 1b — Guard: must return 0 rows.
-- If any row appears, that org has an explicit show_mrp — DROP it from the update list and stop.

select organization_id, purchase_settings->>'show_mrp' as show_mrp
from settings
where organization_id in (
  ...
  and purchase_settings->>'show_mrp' is not null;
