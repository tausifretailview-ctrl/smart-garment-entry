...
BEGIN
  ...
  LOOP
    ...
    IF NOT p_dry_run THEN
      ...
      SELECT c.new_paid, c.new_status INTO v_settled
      FROM public.compute_sale_settlement(v_target.id, p_organization_id) c;
      IF v_settled.new_paid IS NOT NULL THEN
        UPDATE public.sales s
        SET paid_amount = v_settled.new_paid,
            payment_status = v_settled.new_status,
            updated_at = now()
