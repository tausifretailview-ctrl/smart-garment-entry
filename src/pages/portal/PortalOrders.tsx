...
export default function PortalOrders() {
  ...
  const loadItems = useCallback(async (orderId: string) => {
    ...
    const { data } = await supabase
      ...
      .select('product_name, size, quantity, unit_price, line_total')
      .eq('order_id', orderId)
      .is('deleted_at', null);
                    ...
                    {itemsMap[order.id].map((item, i) => (
                      <div key={i} className="px-4 py-2 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">Size: {item.size} · Qty: {item.quantity} · ₹{item.unit_price}/pc</p>
                        </div>
