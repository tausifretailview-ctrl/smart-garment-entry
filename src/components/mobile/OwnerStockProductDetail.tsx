...
export const OwnerStockProductDetail = ({ productId, onBack }: Props) => {
  ...
  const { data: movements } = useQuery({
    queryKey: ["owner-stock-movements", productId, variants?.length],
    queryFn: async () => {
        ...
        <Card className="border-border/40">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {[product.brand, product.product_type, product.style].filter(Boolean).join(" • ") || "—"}
            </p>
            {(product.hsn_code || product.gst_per) && (
              <p className="text-[10px] text-muted-foreground mt-1">
                {product.hsn_code ? `HSN: ${product.hsn_code}` : ""}{product.hsn_code && product.gst_per ? " • " : ""}{product.gst_per ? `GST: ${product.gst_per}%` : ""}
              </p>
            )}
      ...
      <div className="px-4 mt-4">
        <Card className="border-border/40">
          <CardHeader className="pb-2 px-4 pt-4">
            <CardTitle className="text-sm font-semibold">Variants ({variants?.length || 0})</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-3">
            {variantsLoading ? (
              ...
              <div className="space-y-2">
                {variants.map((v, idx) => (
                  <div key={v.id} className={cn("rounded-xl p-3", idx % 2 === 0 ? "bg-muted/20" : "bg-muted/40")}>
                    <div className="flex justify-between items-start">
                      <div className="min-w-0 flex-1">
                        ...
                        {v.barcode && <p className="text-[10px] text-muted-foreground mt-0.5">Barcode: {v.barcode}</p>}
                        <p className="text-[10px] text-muted-foreground mt-0.5">
                          Pur: {fmt(Number(v.pur_price) || 0)} • Sale: {fmt(Number(v.sale_price) || 0)}
                        </p>
                      </div>
            ...
            {movements && movements.length > 0 ? (
              <div className="space-y-0">
                {movements.map((m, idx) => {
                  ...
                  return (
                    <div key={m.id} className={cn("flex items-center gap-2.5 py-2.5", idx < movements.length - 1 && "border-b border-border/40")}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground truncate">
                          {m.bill_number || "—"} • {format(new Date(m.created_at), "dd MMM, hh:mm a")}
                        </p>
                      </div>
