...
describe("buildTrendzoPaymentLines / formatTrendzoPaymentModeLabel", () => {
  it("prints mix CASH + CREDIT amounts on one pair-row (REHMANI POS/26-27/17)", () => {
    ...
    expect(formatTrendzoPaymentModeLabel(lines)).toBe("CASH + CREDIT");
    expect(trendzoMixAmountPair(lines)).toEqual({
      left: "CASH ₹2,000.00",
      right: "CREDIT ₹2,500.00",
    });
  });

  it("splits 4 mix modes across one pair-row", () => {
    const lines = buildTrendzoPaymentLines({
      cashPaid: 500,
      upiPaid: 1000,
      cardPaid: 1500,
    ...
    expect(formatTrendzoPaymentModeLabel(lines)).toBe("CASH + UPI + CARD + CREDIT");
    expect(trendzoMixAmountPair(lines)).toEqual({
      left: "CASH ₹500.00 + UPI ₹1,000.00",
      right: "CARD ₹1,500.00 + CREDIT ₹500.00",
    });
  });

  it("falls back to paymentMethod name when no breakdown; skips mix amount row", () => {
    const lines = buildTrendzoPaymentLines({ paidAmount: 4500, paymentMethod: "cash" });
    expect(formatTrendzoPaymentModeLabel(lines)).toBe("CASH");
    expect(trendzoMixAmountPair(lines)).toBeNull();
  });

  it("falls back to paymentMethod name when nothing is paid", () => {
    expect(formatTrendzoPaymentModeLabel([], "pay_later")).toBe("PAY LATER");
    expect(trendzoMixAmountPair([])).toBeNull();
  });
});
